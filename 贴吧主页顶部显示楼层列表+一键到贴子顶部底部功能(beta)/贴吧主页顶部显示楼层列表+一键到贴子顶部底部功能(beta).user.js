// ==UserScript==
// @name         贴吧主页顶部显示楼层列表+一键到贴子顶部底部功能(beta)
// @namespace    http://tampermonkey.net/
// @version      0.31
// @description  让电脑端贴吧使用起来更便利点.增加了顶部楼层列表，跳转按钮,一键到贴子顶部底部功能
...
/*鼠标移动到按钮上显示的样式*/
.miaocss01:hover,.miaocss02:hover,.miaocss03:hover
{
background: #fff;
color: #3e89fa;
    ...
    function louceliebiao()
    {
        try
        {
            ...
            if(hrefs.split("?")[0].split("/")[3]=="f"||hrefs.indexOf("tab=album")==-1)//解决非主题列表和贴吧图片区报错
            {
                ...
                $("div.pagination-default").children("a").each(function()//主题贴列表
                                                               {
                    let a=false;
                ...
                //2--------------------------------------------------------
                $("div.frs_good_nav_wrap").children("span").children("a").each(function()//精品区列表按钮
                                                                               {
                    //alert("2233");
                    let a=false;
                    for(let i=0;i<$(this)[0].classList.length;i++)
                ...
                //3--------------------------------------------------
                $("input.miaojump2").each(function()//跳转楼层
                                          {
                    let a=false;
                    for(let i=0;i<$(this)[0].classList.length;i++)
                    {
        ...
        try
        {
            ...
            if($("input.miaotop")[0]==null&&$("input.miaobottom")[0]==null)//主题贴列表滚动条到底
            {
                ...
                $("input.miaotop").click(function()
                                         {
                    window.scrollTo(0,0);//到顶
                });
                $("input.miaobottom").click(function()//到底
                                            {
